...
describe('event registration API routing', () => {
  it('loads concerts from the event-registration module', async () => {
    ...
    expect(String(fetchMock.mock.calls[0][0])).toBe('/api/event-registration/concerts')
  })
  ...
  it('scopes ticket lookup and check-in to the selected concert', async () => {
    ...
    await registrationApi.checkIn(1, 2, 'concert-a')
