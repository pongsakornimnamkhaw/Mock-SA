...
async function request<T>(path: string, signal?: AbortSignal): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(body?.error || `Venue/seat API returned ${response.status}`)
  }
...
export default function VenueSeatsViewPage() {
  const [concerts, setConcerts] = useState<ConcertPlan[]>([])
  const [layout, setLayout] = useState<ConcertLayout>({ zones: [], layoutObjects: [] })
  ...
  const availableConcerts = concerts
  ...
  useEffect(() => {
    ...
    let active = true
    setIsLoading(true)
    request<ConcertPlan[]>('/concerts', controller.signal)
      .then((items) => { if (active) setConcerts(items || []) })
      .catch((reason: Error) => {
  ...
  useEffect(() => {
    ...
    setError('')
    request<ConcertLayout>(`/concerts/${encodeURIComponent(selectedConcertId)}/layout`, controller.signal)
      .then((value) => { if (active) setLayout({ zones: value.zones || [], layoutObjects: value.layoutObjects || [], flowchart: value.flowchart }) })
      .catch((reason: Error) => {
        if (active && reason.name !== 'AbortError') setError(reason.message)
      })
        ...
        <Paper variant="outlined" sx={{ minHeight: 420, borderRadius: 3, display: 'grid', placeItems: 'center' }}>
          <Box sx={{ textAlign: 'center', color: '#777b91' }}>
            ...
            <Typography sx={{ fontWeight: 600, color: '#55596e', fontSize: 18, lineHeight: 1.5 }}>
              {searchQuery.trim()
                ? `ไม่พบคอนเสิร์ต “${searchQuery.trim()}”`
                : 'ยังไม่มีรายการคอนเสิร์ต'}
            </Typography>
          </Box>
        </Paper>
      ) : (
