...
func (h *ArtistHandler) history(kind, id, action, description string) {
	h.db.Create(&models.ArtistHistory{EntityType: kind, EntityID: id, Action: action, Description: description})
}
...
func (h *ArtistHandler) deleteArtist(c *fiber.Ctx) error {
	...
	err := h.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Create(&models.ArtistHistory{EntityType: "artist", EntityID: id, Action: "DELETE", Description: fmt.Sprintf("ลบข้อมูลศิลปิน %s", item.ArtistName)}).Error; err != nil {
			return err
		}
		return nil
	})
